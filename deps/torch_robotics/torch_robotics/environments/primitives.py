...
from deps.torch_robotics.torch_robotics.torch_kinematics_tree.geometrics.quaternion import q_to_rotation_matrix
from deps.torch_robotics.torch_robotics.torch_kinematics_tree.geometrics.utils import transform_point, rotate_point
from deps.torch_robotics.torch_robotics.torch_utils.torch_utils import DEFAULT_TENSOR_ARGS, to_torch, to_numpy, tensor_linspace_v1
...
class PrimitiveShapeField(ABC):
    ...
    def compute_signed_distance(self, x,t=None,current_state=None):
        """
        ...
        """
        return self.compute_signed_distance_impl(x,t,current_state)

    @abstractmethod
    def compute_signed_distance_impl(self, x,t=None,current_state=None):
...
class MultiSphereFieldDynamics(PrimitiveShapeField):

    def __init__(self, centers, radii,dynamics_fn=None,velocity=None,tensor_args=None):
        ...
        self.velocity = to_torch(velocity, **self.tensor_args)
        self.dynamics_fn = dynamics_fn
        # self.current_state=current_state
        # self.use_extra_objects=use_extra_objects
        # breakpoint()
    def update_centers(self, t,current_state=None):
        if self.dynamics_fn is not None and t is not None and current_state is not None:
            prev_centers = to_numpy(self.centers)
            # breakpoint()
            new_centers = self.dynamics_fn(t, prev_centers,to_numpy(current_state) ,to_numpy(self.velocity))
            self.centers = to_torch(new_centers, **self.tensor_args)
    ...
    def compute_signed_distance_impl(self, x, t=None,current_state=None):
        # breakpoint()
        self.update_centers(t,current_state)
        # breakpoint()
...
class MultiSphereField(PrimitiveShapeField):
    ...
    def __init__(self, centers, radii, dynamics_fn=None,tensor_args=None):
        """
        Parameters
        ----------
            centers : numpy array
...
class MultiBoxField(PrimitiveShapeField):
    ...
    def compute_signed_distance_impl(self, x,t=None,current_state=None):
        ...
        sdfs = torch.max(distance_to_centers - self.half_sizes.unsqueeze(0).unsqueeze(2), dim=-1)[0]
        return torch.min(sdfs, dim=-1)[0]
...
class MultiRoundedBoxField(MultiBoxField):
    ...
    def compute_signed_distance_impl(self, x,t=None,current_state=None):
        ...
        # q = distance_to_centers - self.half_sizes.unsqueeze(0).unsqueeze(2) + self.radius.unsqueeze(0).unsqueeze(-1) # for mix , spehre and boxes
...
class ObjectField(PrimitiveShapeField):
    ...
    def compute_signed_distance_impl(self, x,t=None,current_state=None):
        ...
        for field in self.fields:
            sdf_fields.append(field.compute_signed_distance_impl(x_new,t,current_state))
