...
class GridMapSDF:
    ...
    def precompute_sdf(self):
        ...
        points_for_sdf_meshgrid = torch.meshgrid(*basis_ranges)
        if self.dim == 2:
            self.points_for_sdf = torch.stack(points_for_sdf_meshgrid, dim=-1)
        elif self.dim == 3:
    # For 3D, we need to reverse the order to match 'ij' indexing
            self.points_for_sdf = torch.stack(
        [points_for_sdf_meshgrid[2], points_for_sdf_meshgrid[1], points_for_sdf_meshgrid[0]], 
        dim=-1
    )
    ...
    def get_sdf(self, X, **kwargs):
        ...
        X_in_map = torch.max(torch.min(X_in_map, max_idx.unsqueeze(0)), torch.zeros_like(max_idx).unsqueeze(0)) # incomplete
