...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim=16, n_layers=1, act='relu', batch_norm=True):
        ...
        activations = {'relu': nn.ReLU, 'sigmoid': nn.Sigmoid, 'tanh': nn.Tanh, 'leaky_relu': nn.LeakyReLU,
                       'elu': nn.ELU, 'prelu': nn.PReLU, 'softplus': nn.Softplus, 'mish': Mish, # nn.Mish,
                       'identity': nn.Identity,'silu':nn.SiLU
                       }
...
# Mish() if self.mish else nn.SiLU(),
...
class ResidualTemporalBlock(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, cond_embed_dim, n_support_points, latent_emb=None,kernel_size=5):
        super().__init__()
    ...
    def forward(self, x, c,z=None): # z scne encoded 
        '''
...
class ResidualTemporalBlockFiLM(nn.Module):
    def __init__(self, inp_channels, out_channels, cond_embed_dim, n_support_points, kernel_size=5):
        ...
        self.film = nn.Sequential(
            nn.Linear(cond_embed_dim, out_channels * 2),
            Rearrange('b (two c) -> b c two', two=2)
        )
    ...
    def forward(self, x, cond):
        ...
        gamma,beta=film_params.unbind(dim=-1)
