...
from deps.torch_robotics.torch_robotics.torch_utils.torch_utils import to_torch
...
class WeightedL2Smooth(WeightedLoss):
    def __init__(self, weights=None, smoothness_weight=0.1):
        super().__init__(weights)
        self.smoothness_weight = smoothness_weight

    def _loss(self, pred, targ):
        mse_loss = F.mse_loss(pred, targ, reduction='none')
        ...
        velocity_targ=targ[:,:,2:]
        smoothness_loss = F.mse_loss(velocity_pred[:, 1:] - velocity_pred[:, :-1], 
                                             velocity_targ[:, 1:] - velocity_targ[:, :-1],
                                             reduction='none')
        smoothness_loss = F.pad(smoothness_loss, (0, 2, 0, 1), mode='constant', value=0)
        # Combine losses
        # breakpoint()
        combined_loss = 0.9*mse_loss + self.smoothness_weight * smoothness_loss
        return combined_loss
Losses = {
    'l1': WeightedL1,
    'l2': WeightedL2,
    'l2smooth':WeightedL2Smooth,
}
